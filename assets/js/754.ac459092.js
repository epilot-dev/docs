...
var isDescendant = /* @__PURE__ */ (0,chunk_AGHRB4JF/* __name */.eW)((id, ancestorId) => {
  ...
  chunk_AGHRB4JF/* log.trace */.cM.trace("In isDescendant", ancestorId, " ", id, " = ", ancestorDescendants.includes(id));
  return ancestorDescendants.includes(id);
}, "isDescendant");
var edgeInCluster = /* @__PURE__ */ (0,chunk_AGHRB4JF/* __name */.eW)((edge, clusterId) => {
  const clusterDescendants = descendants.get(clusterId) || [];
  chunk_AGHRB4JF/* log.info */.cM.info("Descendants of ", clusterId, " is ", clusterDescendants);
  chunk_AGHRB4JF/* log.info */.cM.info("Edge is ", edge);
  if (edge.v === clusterId || edge.w === clusterId) {
    return false;
  }
  if (!clusterDescendants) {
    chunk_AGHRB4JF/* log.debug */.cM.debug("Tilt, ", clusterId, ",not in descendants");
    return false;
  }
  return clusterDescendants.includes(edge.v) || isDescendant(edge.v, clusterId) || isDescendant(edge.w, clusterId) || clusterDescendants.includes(edge.w);
}, "edgeInCluster");
var copy = /* @__PURE__ */ (0,chunk_AGHRB4JF/* __name */.eW)((clusterId, graph, newGraph, rootId) => {
  chunk_AGHRB4JF/* log.warn */.cM.warn(
    "Copying children of ",
    clusterId,
    "root",
    rootId,
  ...
  chunk_AGHRB4JF/* log.warn */.cM.warn("Copying (nodes) clusterId", clusterId, "nodes", nodes);
  nodes.forEach((node) => {
    if (graph.children(node).length > 0) {
      copy(node, graph, newGraph, rootId);
    } else {
      const data = graph.node(node);
      chunk_AGHRB4JF/* log.info */.cM.info("cp ", node, " to ", rootId, " with parent ", clusterId);
      newGraph.setNode(node, data);
      if (rootId !== graph.parent(node)) {
        chunk_AGHRB4JF/* log.warn */.cM.warn("Setting parent", node, graph.parent(node));
        newGraph.setParent(node, graph.parent(node));
      }
      if (clusterId !== rootId && node !== clusterId) {
        chunk_AGHRB4JF/* log.debug */.cM.debug("Setting parent", node, clusterId);
        newGraph.setParent(node, clusterId);
      } else {
        chunk_AGHRB4JF/* log.info */.cM.info("In copy ", clusterId, "root", rootId, "data", graph.node(clusterId), rootId);
        chunk_AGHRB4JF/* log.debug */.cM.debug(
      ...
      chunk_AGHRB4JF/* log.debug */.cM.debug("Copying Edges", edges);
      edges.forEach((edge) => {
        chunk_AGHRB4JF/* log.info */.cM.info("Edge", edge);
        const data2 = graph.edge(edge.v, edge.w, edge.name);
        chunk_AGHRB4JF/* log.info */.cM.info("Edge data", data2, rootId);
        try {
          if (edgeInCluster(edge, rootId)) {
            chunk_AGHRB4JF/* log.info */.cM.info("Copying as ", edge.v, edge.w, data2, edge.name);
            newGraph.setEdge(edge.v, edge.w, data2, edge.name);
            chunk_AGHRB4JF/* log.info */.cM.info("newGraph edges ", newGraph.edges(), newGraph.edge(newGraph.edges()[0]));
          } else {
            chunk_AGHRB4JF/* log.info */.cM.info(
          ...
          chunk_AGHRB4JF/* log.error */.cM.error(e);
        }
      });
    }
    chunk_AGHRB4JF/* log.debug */.cM.debug("Removing node", node);
...
var findNonClusterChild = /* @__PURE__ */ (0,chunk_AGHRB4JF/* __name */.eW)((id, graph, clusterId) => {
  const children = graph.children(id);
  chunk_AGHRB4JF/* log.trace */.cM.trace("Searching children of id ", id, children);
...
var adjustClustersAndEdges = /* @__PURE__ */ (0,chunk_AGHRB4JF/* __name */.eW)((graph, depth) => {
  if (!graph || depth > 10) {
    chunk_AGHRB4JF/* log.debug */.cM.debug("Opting out, no graph ");
    return;
  } else {
    chunk_AGHRB4JF/* log.debug */.cM.debug("Opting in, graph ");
  }
  ...
  graph.nodes().forEach(function(id) {
    ...
    if (children.length > 0) {
      chunk_AGHRB4JF/* log.debug */.cM.debug("Cluster identified", id, descendants);
      edges.forEach((edge) => {
        const d1 = isDescendant(edge.v, id);
        const d2 = isDescendant(edge.w, id);
        if (d1 ^ d2) {
          chunk_AGHRB4JF/* log.warn */.cM.warn("Edge: ", edge, " leaves cluster ", id);
          chunk_AGHRB4JF/* log.warn */.cM.warn("Descendants of XXX ", id, ": ", descendants.get(id));
          clusterDb.get(id).externalConnections = true;
        }
      });
    } else {
      chunk_AGHRB4JF/* log.debug */.cM.debug("Not a cluster ", id, descendants);
    }
  });
  ...
  graph.edges().forEach(function(e) {
    ...
    chunk_AGHRB4JF/* log.warn */.cM.warn("Edge " + e.v + " -> " + e.w + ": " + JSON.stringify(e));
    chunk_AGHRB4JF/* log.warn */.cM.warn("Edge " + e.v + " -> " + e.w + ": " + JSON.stringify(graph.edge(e)));
    let v = e.v;
    let w = e.w;
    chunk_AGHRB4JF/* log.warn */.cM.warn(
      "Fix XXX",
    ...
    if (clusterDb.get(e.v) || clusterDb.get(e.w)) {
      chunk_AGHRB4JF/* log.warn */.cM.warn("Fixing and trying - removing XXX", e.v, e.w, e.name);
      v = getAnchorId(e.v);
      w = getAnchorId(e.w);
      ...
      chunk_AGHRB4JF/* log.warn */.cM.warn("Fix Replacing with XXX", v, w, e.name);
      graph.setEdge(v, w, edge, e.name);
    }
  });
  chunk_AGHRB4JF/* log.warn */.cM.warn("Adjusted Graph", write(graph));
  extractor(graph, 0);
  chunk_AGHRB4JF/* log.trace */.cM.trace(clusterDb);
}, "adjustClustersAndEdges");
var extractor = /* @__PURE__ */ (0,chunk_AGHRB4JF/* __name */.eW)((graph, depth) => {
  chunk_AGHRB4JF/* log.warn */.cM.warn("extractor - ", depth, write(graph), graph.children("D"));
  if (depth > 10) {
    chunk_AGHRB4JF/* log.error */.cM.error("Bailing out");
    return;
  }
  ...
  if (!hasChildren) {
    chunk_AGHRB4JF/* log.debug */.cM.debug("Done, no node has children", graph.nodes());
    return;
  }
  chunk_AGHRB4JF/* log.debug */.cM.debug("Nodes = ", nodes, depth);
  for (const node of nodes) {
    chunk_AGHRB4JF/* log.debug */.cM.debug(
      "Extracting node",
      node,
    ...
    if (!clusterDb.has(node)) {
      chunk_AGHRB4JF/* log.debug */.cM.debug("Not a cluster", node, depth);
      ...
      if (clusterDb.get(node)?.clusterData?.dir) {
        ...
        chunk_AGHRB4JF/* log.warn */.cM.warn("Fixing dir", clusterDb.get(node).clusterData.dir, dir);
      }
      ...
      chunk_AGHRB4JF/* log.warn */.cM.warn("Old graph before copy", write(graph));
      ...
      chunk_AGHRB4JF/* log.warn */.cM.warn("New graph after copy node: (", node, ")", write(clusterGraph));
      chunk_AGHRB4JF/* log.debug */.cM.debug("Old graph after copy", write(graph));
    } else {
      chunk_AGHRB4JF/* log.warn */.cM.warn(
        "Cluster ** ",
        node,
      ...
      chunk_AGHRB4JF/* log.debug */.cM.debug(clusterDb);
    }
  }
  nodes = graph.nodes();
  chunk_AGHRB4JF/* log.warn */.cM.warn("New list of nodes", nodes);
  for (const node of nodes) {
    const data = graph.node(node);
    chunk_AGHRB4JF/* log.warn */.cM.warn(" Now next level", node, data);
    if (data?.clusterNode) {
...
var recursiveRender = /* @__PURE__ */ (0,chunk_AGHRB4JF/* __name */.eW)(async (_elem, graph, diagramType, id, parentCluster, siteConfig) => {
  chunk_AGHRB4JF/* log.warn */.cM.warn("Graph in recursive render:XAX", write(graph), parentCluster);
  const dir = graph.graph().rankdir;
  chunk_AGHRB4JF/* log.trace */.cM.trace("Dir in recursive render - dir:", dir);
  const elem = _elem.insert("g").attr("class", "root");
  if (!graph.nodes()) {
    chunk_AGHRB4JF/* log.info */.cM.info("No nodes found for", graph);
  } else {
    chunk_AGHRB4JF/* log.info */.cM.info("Recursive render XXX", graph.nodes());
  }
  if (graph.edges().length > 0) {
    chunk_AGHRB4JF/* log.info */.cM.info("Recursive edges", graph.edge(graph.edges()[0]));
  }
  ...
  await Promise.all(
    graph.nodes().map(async function(v) {
      ...
      if (parentCluster !== void 0) {
        ...
        chunk_AGHRB4JF/* log.trace */.cM.trace(
        ...
        if (!graph.parent(v)) {
          chunk_AGHRB4JF/* log.trace */.cM.trace("Setting parent", v, parentCluster.id);
          graph.setParent(v, parentCluster.id, data);
        }
      }
      chunk_AGHRB4JF/* log.info */.cM.info("(Insert) Node XXX" + v + ": " + JSON.stringify(graph.node(v)));
      if (node?.clusterNode) {
        chunk_AGHRB4JF/* log.info */.cM.info("Cluster identified XBX", v, node.width, graph.node(v));
        ...
        if (graph.children(v).length > 0) {
          ...
          chunk_AGHRB4JF/* log.trace */.cM.trace(findNonClusterChild(node.id, graph));
          clusterDb.set(node.id, { id: findNonClusterChild(node.id, graph), node });
        } else {
          chunk_AGHRB4JF/* log.trace */.cM.trace("Node - the non recursive path XAX", v, nodes, graph.node(v), dir);
  ...
  const processEdges = /* @__PURE__ */ (0,chunk_AGHRB4JF/* __name */.eW)(async () => {
    const edgePromises = graph.edges().map(async function(e) {
      ...
      chunk_AGHRB4JF/* log.info */.cM.info("Edge " + e.v + " -> " + e.w + ": " + JSON.stringify(e));
      chunk_AGHRB4JF/* log.info */.cM.info("Edge " + e.v + " -> " + e.w + ": ", e, " ", JSON.stringify(graph.edge(e)));
      chunk_AGHRB4JF/* log.info */.cM.info(
        "Fix",
        clusterDb,
        "ids:",
  ...
  await processEdges();
  chunk_AGHRB4JF/* log.info */.cM.info("Graph before layout:", JSON.stringify(write(graph)));
  chunk_AGHRB4JF/* log.info */.cM.info("############################################# XXX");
  chunk_AGHRB4JF/* log.info */.cM.info("###                Layout                 ### XXX");
  chunk_AGHRB4JF/* log.info */.cM.info("############################################# XXX");
  (0,dagre/* layout */.bK)(graph);
  chunk_AGHRB4JF/* log.info */.cM.info("Graph after layout:", JSON.stringify(write(graph)));
  let diff = 0;
    ...
    sortNodesByHierarchy(graph).map(async function(v) {
      ...
      if (node?.clusterNode) {
        ...
        if (graph.children(v).length > 0) {
          ...
          chunk_AGHRB4JF/* log.debug */.cM.debug("OffsetY", offsetY, "labelHeight", labelHeight, "halfPadding", halfPadding);
  ...
  graph.edges().forEach(function(e) {
    const edge = graph.edge(e);
    chunk_AGHRB4JF/* log.info */.cM.info("Edge " + e.v + " -> " + e.w + ": " + JSON.stringify(edge), edge);
  ...
  graph.nodes().forEach(function(v) {
    ...
    chunk_AGHRB4JF/* log.info */.cM.info(v, n.type, n.diff);
    if (n.isGroup) {
      diff = n.diff;
    }
  });
  chunk_AGHRB4JF/* log.warn */.cM.warn("Returning from recursive render XAX", elem, diff);
  return { elem, diff };
}, "recursiveRender");
var render = /* @__PURE__ */ (0,chunk_AGHRB4JF/* __name */.eW)(async (data4Layout, svg) => {
  ...
  chunk_AGHRB4JF/* log.debug */.cM.debug("Edges:", data4Layout.edges);
  ...
  chunk_AGHRB4JF/* log.warn */.cM.warn("Graph at first:", JSON.stringify(write(graph)));
  adjustClustersAndEdges(graph);
  chunk_AGHRB4JF/* log.warn */.cM.warn("Graph after XAX:", JSON.stringify(write(graph)));
