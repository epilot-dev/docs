...
It's used when you want to charge your customers for a product based on the amount of units they consume. It's necessary to define the price tiers on `Price` entity.
The amount to be paid by the customer will consider unit price of the tier according to the quantity consumed and the same unit price is applied for all the consumed units.
...
It's used when you want to charge your customers for a product based on the amount of units they consume. It's necessary to define the price tiers on `Price` entity.
While similar to tiered volume, tiered flat fee charges for the same quantity for the entire range instead using the unit price to multiply the quantity.
