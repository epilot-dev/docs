...
The mapping input can be requested on a [Journey](/docs/journeys/journey-builder) via a NumberInputBlock mapped to the price or on a [Order](/docs/pricing/orders) via the mapping input.

The quantity input can be requested on a [Journey](/docs/journeys/journey-builder) via a quantity input on a ProductSelectBlock/ShoppingCart or on a [Order](/docs/pricing/orders) via the quantity input.
